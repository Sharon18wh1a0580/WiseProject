...
public class HibernateTemplate {
	...
	/*public static List<Products1> viewMyProducts(int custId)
	{
		System.out.println("Hibernate "+custId);
		String queryS="select p.productName, p.description,p.category,p.quantityAdded,p.price from Products p,OrderHistory o where p.productId = o.productId and custId=:custId";
		List<Products1> li= (List)sessionFactory.openSession().createQuery(queryS).setParameter("custId", custId).list();	
		return li;
	...
	public static double repeatOrder(int custId) {
		  System.out.println("hiiii....");
	      SessionFactory s = new Configuration().configure().buildSessionFactory();
			Session session = s.openSession();
			session.beginTransaction();
	...
	//customer login
	public static Object customerLogin(String loginId, String password) {
			String queryString = "from Customer where loginId = :loginId and password = :password";
			Query query = sessionFactory.openSession().createQuery(queryString);
	...
	public static void deletePro(int productId) {
		deleteObject(Products.class, productId);
	}
